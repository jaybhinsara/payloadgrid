...
export function PublicPage({ eyebrow, title, intro, children }: { eyebrow: string; title: string; intro: string; children: ReactNode }) {
  return <main className="public-shell"><PublicHeader /><header className="public-page-hero"><span className="section-label">{eyebrow}</span><h1>{title}</h1><p>{intro}</p></header><div className="public-page-body">{children}</div><PublicFooter /></main>;
}
