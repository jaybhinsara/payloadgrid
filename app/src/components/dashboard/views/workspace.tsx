...
export function WorkspaceView({ data, busy, submit, mutate, switchOrganization, switchProject }: { data: DashboardData; busy: string; submit: DashboardSubmit; mutate: DashboardMutate; switchOrganization: (id: string) => Promise<void>; switchProject: (id: string) => Promise<void> }) {
  ...
  return <>
    ...
    {canAdminister ? <CircuitBreakerPanel data={data} mutate={mutate} /> : null}
    ...
    {canAdminister ? <section className="content-card workspace-admin"><div className="card-head"><div><span className="section-label">Workspace administration</span><h3>Identity and ownership</h3></div><ShieldCheck size={18} /></div><div className="workspace-admin-grid"><form onSubmit={renameWorkspace}><label>Workspace name<input name="name" required defaultValue={data.context.organization.name} /></label><button className="button secondary"><Pencil size={14} /> Rename workspace</button></form>{isOwner ? <form onSubmit={transferOwnership}><label>Transfer ownership<select name="userId" required defaultValue="" disabled={!otherMembers.length}><option value="" disabled>{otherMembers.length ? "Select a member" : "No eligible members"}</option>{otherMembers.map((member) => <option key={member.id} value={member.id}>{member.name} · {member.role}</option>)}</select></label><button className="button secondary" disabled={!otherMembers.length} title={!otherMembers.length ? "Invite a team member before transferring ownership" : "Transfer workspace ownership"}><UserRoundCog size={14} /> Transfer</button>{!otherMembers.length ? <small className="form-help">Invite a team member before transferring ownership.</small> : null}</form> : null}</div></section> : null}
...
function CircuitBreakerPanel({ data, mutate }: { data: DashboardData; mutate: DashboardMutate }) {
  const [endpointId, setEndpointId] = useState(data.endpoints[0]?.id || "");
  const endpoint = data.endpoints.find((item) => item.id === endpointId);
  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    await mutate(`/api/endpoints/${endpointId}`, { circuitBreakerEnabled: form.get("enabled") === "on", circuitBreakerThreshold: Number(form.get("threshold") || 100) }, "PATCH");
  }
  return <section className="content-card circuit-panel"><div className="card-head"><div><span className="section-label">Traffic protection</span><h3>Anomaly circuit breakers</h3></div><ShieldCheck size={18} /></div><p>When inbound traffic exceeds the configured floor and its recent baseline, new deliveries move into the buffer until an operator resumes the path.</p>{data.endpoints.length ? <form onSubmit={save}><label>Endpoint<select value={endpointId} onChange={(event) => setEndpointId(event.target.value)}>{data.endpoints.map((item) => <option value={item.id} key={item.id}>{item.name}{item.circuit_state === "open" ? " · circuit open" : ""}</option>)}</select></label><label>Spike floor per minute<input name="threshold" type="number" min="20" max="100000" defaultValue={endpoint?.circuit_breaker_threshold || 100} key={`${endpointId}-threshold`} /></label><div className="circuit-toggle"><span>Protection mode</span><label className="check-control"><input name="enabled" type="checkbox" defaultChecked={endpoint?.circuit_breaker_enabled} key={`${endpointId}-enabled`} /><span><strong>Automatic buffering enabled</strong><small>Uses both this floor and the route&apos;s recent deviation.</small></span></label></div><div className="circuit-actions"><button className="button primary"><ShieldCheck size={15} /> Save protection</button>{endpoint?.circuit_state === "open" ? <button type="button" className="button secondary" onClick={() => void mutate(`/api/endpoints/${endpoint.id}`, { circuitState: "closed" }, "PATCH")}><ArrowRightLeft size={15} /> Resume buffered traffic</button> : null}</div></form> : <p className="form-copy">Create an endpoint before configuring traffic protection.</p>}</section>;
}
...
function ProjectEditor({ project, mutate, close }: { project: Project; mutate: DashboardMutate; close: () => void }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);
    const result = await mutate(`/api/projects/${project.id}`, { name: form.get("name"), environment: form.get("environment"), payloadRetentionMode: form.get("payloadRetentionMode") }, "PATCH");
    setSaving(false); if (result) close();
  }
  return <div className="drawer-backdrop" onMouseDown={close}><aside className="endpoint-drawer" onMouseDown={(event) => event.stopPropagation()}><header><div><span className="section-label">Project settings</span><h2>{project.name}</h2><p>{project.slug}</p></div><button className="icon-button" onClick={close} aria-label="Close project settings"><X size={19} /></button></header><form onSubmit={save}><label>Project name<input name="name" required defaultValue={project.name} /></label><label>Environment<select name="environment" defaultValue={project.environment}><option value="development">Development</option><option value="staging">Staging</option><option value="production">Production</option></select></label><label>Payload retention<select name="payloadRetentionMode" defaultValue={project.payloadRetentionMode}><option value="standard">Standard · retain for diagnostics</option><option value="transient">Transient · scrub after terminal delivery</option></select><small>Transient mode keeps payload data only while delivery or retries are active. Headers and delivery evidence remain.</small></label><button className="button primary" disabled={saving}>{saving ? <LoaderCircle className="spin" size={16} /> : <Pencil size={15} />} Save project</button></form></aside></div>;
}
