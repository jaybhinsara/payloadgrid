...
export function UsageView({ data }: { data: DashboardData }) {
  ...
  const rows = [
    { label: "Accepted events", value: data.usage.acceptedEvents, limit: limits.messagesPerMonth, icon: Activity, copy: "Inbound events and outbound API messages across this workspace" },
  ...
  return <>
    ...
    <section className="usage-summary">
      <article><span><Gauge size={18} /></span><div><small>Plan</small><strong>{data.usage.plan.name}</strong></div></article>
    ...
    <section className="content-card usage-card"><div className="card-head"><div><span className="section-label">Included capacity</span><h3>Plan limits</h3></div><span>{data.system.queueConfigured ? "Durable queue active" : "Fallback delivery"}</span></div><div className="usage-list">{rows.map(({ label, value, limit, icon: Icon, copy }) => { const percent = Math.min(100, Math.round((value / limit) * 100)); return <article key={label}><span className="resource-icon"><Icon size={17} /></span><div><div><strong>{label}</strong><span>{value.toLocaleString()} / {limit.toLocaleString()}</span></div><p>{copy}</p><div className="usage-track"><i style={{ width: `${percent}%` }} /></div></div><em>{percent}%</em></article>; })}</div></section>
    ...
    {data.usage.plan.id === "free" ? <section className="content-card billing-callout"><div><span className="section-label">Paid capacity</span><h3>Need more events or retention?</h3><p>Starter includes 100,000 accepted events, 30 endpoints, 10 members, and 14-day payload retention for $29 per workspace each month.</p></div><Link className="button primary" href="/contact?plan=starter">Request Starter</Link></section> : null}
  </>;
}
