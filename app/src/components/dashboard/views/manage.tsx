...
export function ApiKeysView({ data, busy, submit, revoke, reveal }: { data: DashboardData; busy: string; submit: DashboardSubmit; revoke: (id: string) => void; reveal: (value: string) => void }) {
  return <><SectionHead eyebrow="Developer access" heading="API keys" copy="Keys authenticate server-to-server requests. PayloadGrid stores only a one-way hash." /><section className="split-layout"><form className="content-card form-card" onSubmit={(event) => submit(event, "/api/api-keys", (form) => ({ name: form.get("name"), scopes: form.getAll("scopes") }), (payload) => reveal(String(payload.token || "")))}><h3>Create API key</h3><label>Key name<input name="name" required placeholder="Production server" /></label><fieldset><legend>Permissions</legend><label className="check-control"><input name="scopes" type="checkbox" value="messages:write" defaultChecked /><span><strong>Send messages</strong><small>Single and batch delivery APIs.</small></span></label><label className="check-control"><input name="scopes" type="checkbox" value="events:read" /><span><strong>Read relay events</strong><small>Local relay and event inspection.</small></span></label><label className="check-control"><input name="scopes" type="checkbox" value="embeds:write" /><span><strong>Create embed sessions</strong><small>Issue short-lived customer portal URLs.</small></span></label></fieldset><button className="button primary" disabled={busy === "/api/api-keys"}><KeyRound size={16} /> Generate key</button></form><section className="content-card list-card">{data.apiKeys.length ? <div className="resource-list">{data.apiKeys.map((key) => <article key={key.id} className={key.revoked_at ? "disabled" : ""}><span className="resource-icon"><KeyRound size={18} /></span><div><strong>{key.name}</strong><code>{key.key_prefix}••••••••••••</code><small>{key.scopes.join(" · ")} · Last used {timeAgo(key.last_used_at)}</small></div>{key.revoked_at ? <span>Revoked</span> : <button className="button danger small" onClick={() => revoke(key.id)}>Revoke</button>}</article>)}</div> : <Empty icon={<KeyRound size={22} />} title="No API keys" copy="Generate a key to send messages from your backend." />}</section></section></>;
}
