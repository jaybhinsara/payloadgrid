...
export type View = "overview" | "applications" | "endpoints" | "messages" | "deliveries" | "event-types" | "api-keys" | "team" | "usage" | "settings" | "workspace" | "operations";
export type Application = { id: string; name: string; uid: string; description: string | null; created_at: string };
export type Endpoint = { id: string; application_id: string | null; name: string; provider: string; destination_url: string; signing_secret: string | null; provider_verification_required: boolean; provider_secret_hint: string | null; delivery_header_names: string[]; is_active: boolean; circuit_breaker_enabled: boolean; circuit_breaker_threshold: number; circuit_state: "closed" | "open"; circuit_opened_at: string | null; event_types: string[]; created_at: string };
export type EventRow = { id: string; endpoint_id: string; endpoint_name: string; application_id: string | null; message_id: string | null; direction: string; provider: string; provider_event_id: string | null; event_type: string; status: string; revenue_at_risk: number; revenue_currency: string | null; received_at: string; response_status: number | null; response_body: string | null; latency_ms: number | null; error: string | null; retry_count: number; max_retries: number; next_retry_at: string | null; last_error: string | null; cancelled_at: string | null; dead_lettered_at: string | null; resolved_at: string | null; resolution_note: string | null; resolved_by_name: string | null; attempt_count: number; request_headers: Record<string, unknown>; request_body: unknown; request_content_type: string; is_simulation: boolean; parent_event_id: string | null };
...
export type DashboardData = {
  ok: boolean; error?: string; appUrl: string; system: { queueConfigured: boolean; operator: boolean }; usage: { periodStart: string; acceptedEvents: number; limits: { messagesPerMonth: number; apiRequestsPerMinute: number; inboundRequestsPerMinute: number; endpoints: number; teamMembers: number; payloadRetentionDays: number } };
