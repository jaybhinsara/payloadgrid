...
export function StatusIcon({ status }: { status: string }) { return ["delivered", "resolved"].includes(status) ? <CircleCheck size={15} /> : ["failed", "dead_letter", "cancelled"].includes(status) ? <CircleX size={15} /> : <RefreshCw size={14} />; }
export function Status({ value }: { value: string }) { return <i className={`status-pill ${value}`}><StatusIcon status={value} />{value.replaceAll("_", " ")}</i>; }
