...
export default function DocsPage() {
  return <PublicPage eyebrow="Developer documentation" title="Integrate webhooks without guessing." intro="Start with one authenticated request, then use the same delivery records, signatures, retries, and provider routes in production.">
    <div className="docs-layout"><aside className="docs-nav"><strong>GET STARTED</strong><a href="#quickstart">Quickstart</a><a href="#model">Data model</a><a href="#outbound">Send webhooks</a><a href="#batch">Batch ingestion</a><a href="#inbound">Receive webhooks</a><a href="#relay">Local relay</a><a href="#embed">React embed</a><a href="#signatures">Verify signatures</a><a href="#providers">Provider setup</a><a href="#retries">Retries</a><a href="#operations">Operational controls</a><a href="#limits">Plan limits</a><a href="#api-reference">API reference</a></aside>
      <article className="docs-content">
        ...
        <section id="embed"><span className="section-label">07 / React embed</span><h2>Keep delivery history inside your product</h2><p>Create a server-side API key with only `embeds:write`. Exchange it for a short-lived URL bound to one application and an explicit permission set. Never expose the API key in browser code.</p><h3>Server</h3><CodeBlock>{embedServerExample}</CodeBlock><h3>React client</h3><CodeBlock>{embedReactExample}</CodeBlock><div className="docs-callout"><Check size={18} /><p><strong>Least privilege by default.</strong> Use `deliveries:read` for history. Add `deliveries:replay` only when the signed-in customer is allowed to replay events.</p></div></section>
        ...
        <section id="api-reference"><span className="section-label">13 / API reference</span><h2>Current public endpoints</h2><div className="api-reference"><div><code>POST</code><strong>/api/v1/messages</strong><p>Accept one outbound message.</p></div><div><code>POST</code><strong>/api/v1/messages/batch</strong><p>Accept up to 100 outbound messages.</p></div><div><code>GET</code><strong>/api/v1/relay/events</strong><p>Read a project-scoped local relay feed.</p></div><div><code>POST</code><strong>/api/v1/embed-token</strong><p>Create a short-lived embedded portal URL.</p></div><div><code>POST</code><strong>/in/:endpointId</strong><p>Accept and verify an inbound provider callback.</p></div><div><code>GET</code><strong>/api/health</strong><p>Return customer-facing component health.</p></div></div><Link className="button secondary" href="/api/openapi" target="_blank">OpenAPI JSON <ExternalLink size={15} /></Link></section>
      </article>
    </div>
  </PublicPage>;
}
