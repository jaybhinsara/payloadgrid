...
export default function DocsPage() {
  return <PublicPage eyebrow="Developer documentation" title="Integrate webhooks without guessing." intro="Start with one authenticated request, then use the same delivery records, signatures, retries, and provider routes in production.">
    <div className="docs-layout"><aside className="docs-nav"><strong>GET STARTED</strong><a href="#quickstart">Quickstart</a><a href="#model">Data model</a><a href="#outbound">Send webhooks</a><a href="#inbound">Receive webhooks</a><a href="#signatures">Verify signatures</a><a href="#providers">Provider setup</a><a href="#retries">Retries</a><a href="#limits">Beta limits</a><a href="#api-reference">API reference</a></aside>
      <article className="docs-content">
        ...
        <section id="retries"><span className="section-label">07 / Recovery</span><h2>Retry without creating duplicates</h2><p>Failed destinations move through increasing delays of 1, 5, 30, 120, 360, and 720 minutes. After the final attempt, the delivery enters the dead-letter queue. Manual replay creates another attempt on the same delivery record. The event ID and signature identity stay stable across attempts.</p></section>
        <section id="limits"><span className="section-label">08 / Public beta limits</span><h2>Clear limits while the system earns trust</h2><ul className="check-list-public"><li><Check size={16} /> 10,000 accepted events per project each month</li><li><Check size={16} /> 10 endpoints per project</li><li><Check size={16} /> 300 API or inbound requests per minute</li><li><Check size={16} /> 256 KB payloads</li><li><Check size={16} /> 3-day payload retention</li><li><Check size={16} /> No uptime SLA during public beta</li></ul><Link className="text-link" href="/pricing">Read beta pricing <ArrowRight size={15} /></Link></section>
        <section id="api-reference"><span className="section-label">09 / API reference</span><h2>Current public endpoints</h2><div className="api-reference"><div><code>POST</code><strong>/api/v1/messages</strong><p>Accept an outbound message. Bearer API key required.</p></div><div><code>POST</code><strong>/in/:endpointId</strong><p>Accept and verify an inbound provider callback.</p></div><div><code>GET</code><strong>/api/health</strong><p>Return public database and queue health.</p></div></div><Link className="button secondary" href="/api/openapi" target="_blank">OpenAPI JSON <ExternalLink size={15} /></Link></section>
      </article>
    </div>
  </PublicPage>;
}