...
export default async function StatusPage() {
  ...
  return <PublicPage eyebrow="Live service status" title={title} intro="Current availability and incident history for the PayloadGrid services customers depend on.">
    ...
    <section className="status-history"><div className="status-section-head"><div><span className="section-label">Monitoring history</span><h2>Recent service checks</h2></div><span>{monitoring?.monitoringSince ? `Collecting since ${new Date(monitoring.monitoringSince).toLocaleDateString("en-US")}` : "Awaiting first scheduled run"}</span></div>{hasRuns ? <div className="check-run-strip" aria-label="Recent monitoring runs">{monitoring!.recentRuns.map((run) => { const checkedAt = new Date(run.checkedAt); const description = `${statusCopy(run.status as ServiceState)} · ${checkedAt.toLocaleString("en-US")}`; return <span className="check-run" key={run.runId} title={description} aria-label={description}><i className={run.status} /><time dateTime={run.checkedAt}><b>{checkedAt.toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}</b>{checkedAt.toLocaleDateString("en-US", { month: "short", day: "numeric" })}</time></span>; })}</div> : <div className="status-empty"><Gauge size={20} /><div><strong>Monitoring history is being collected</strong><p>Scheduled checks will appear here after the first scheduled monitoring run.</p></div></div>}</section>
