...
async function claims(request: Request, permission: EmbedPermission) {
  ...
  if (!value?.permissions.includes(permission)) return null;
  const sql = requireSql();
  const [active] = await sql`select 1 from projects p join organizations o on o.id=p.organization_id where p.id=${value.projectId} and o.suspended_at is null limit 1`;
  return active ? value : null;
}
export async function GET(request: Request) {
  const value = await claims(request, "endpoints:read"); if (!value) return NextResponse.json({ ok: false, error: "Embed token cannot read endpoints" }, { status: 403 }); const sql = requireSql();
...
export async function POST(request: Request) {
  const value = await claims(request, "endpoints:write"); if (!value) return NextResponse.json({ ok: false, error: "Embed token cannot create endpoints" }, { status: 403 });
...
export async function PATCH(request: Request) {
  try { const body=updateSchema.parse(await request.json()); const permission:EmbedPermission=body.action==="rotate"?"secrets:rotate":"subscriptions:write"; const value=await claims(request,permission); if(!value)return NextResponse.json({ok:false,error:`Embed token cannot ${body.action}`},{status:403}); const sql=requireSql(); const [endpoint]=await sql`select ep.id,p.organization_id from endpoints ep join projects p on p.id=ep.project_id where ep.id=${body.endpointId} and ep.project_id=${value.projectId} and ep.application_id=${value.applicationId} and ep.deleted_at is null`; if(!endpoint)return NextResponse.json({ok:false,error:"Endpoint not found"},{status:404}); if(body.action==="subscriptions"){await sql`delete from endpoint_subscriptions where endpoint_id=${body.endpointId}`; for(const eventType of [...new Set(body.eventTypes||[])])await sql`insert into endpoint_subscriptions(endpoint_id,event_type) values (${body.endpointId},${eventType}) on conflict do nothing`; await writeAudit(String(endpoint.organization_id),null,"embed.endpoint_subscriptions_updated","endpoint",body.endpointId,{eventTypes:body.eventTypes||[]}); return NextResponse.json({ok:true});} const secret=`whsec_${randomToken(24)}`; await sql`update endpoints set previous_signing_secret=signing_secret,previous_signing_secret_expires_at=now()+interval '24 hours',signing_secret=${secret},updated_at=now() where id=${body.endpointId}`; await writeAudit(String(endpoint.organization_id),null,"embed.endpoint_secret_rotated","endpoint",body.endpointId); return NextResponse.json({ok:true,signingSecret:secret,previousValidForHours:24}); }
