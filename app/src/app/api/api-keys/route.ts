...
export async function POST(request: Request) {
  try { const context = await requireSession(); requireRole(context, ["owner", "admin", "developer"]); const body = schema.parse(await request.json()); const sql = requireSql(); const key = createApiKey(); const [record] = await sql`insert into api_keys (project_id, name, key_prefix, key_hash, scopes, created_by) values (${context.project.id}, ${body.name}, ${key.prefix}, ${key.hash}, ${body.scopes}, ${context.user.id}) returning id, name, key_prefix, scopes, created_at`; await writeAudit(context.organization.id, context.user.id, "api_key.created", "api_key", String(record.id), { scopes: body.scopes }); return NextResponse.json({ ok: true, key: record, token: key.token }, { status: 201 }); }
