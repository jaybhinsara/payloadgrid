...
const alertSchema = z.object({ kind: z.literal("alert"), name: z.string().min(2).max(80), channel: z.enum(["email", "slack", "webhook"]), destination: z.string().min(3).max(500), failureThreshold: z.number().int().min(1).max(100).default(3) });
...
export async function POST(request: Request) {
  try { const context = await requireSession(); requireRole(context, ["owner", "admin", "developer"]); const body = schema.parse(await request.json()); const sql = requireSql();
    ...
    const [record] = await sql`insert into alert_rules (project_id, name, channel, destination, failure_threshold) values (${context.project.id}, ${body.name}, ${body.channel}, ${body.destination}, ${body.failureThreshold}) returning id, name, channel, destination, failure_threshold, is_active, created_at`; await writeAudit(context.organization.id, context.user.id, "alert.created", "alert", String(record.id)); return NextResponse.json({ ok: true, record }, { status: 201 });
