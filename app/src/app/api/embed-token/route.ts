...
export async function POST(request: Request) {
  try {
    ...
    const token = createEmbedToken({ projectId: context.project.id, applicationId: body.applicationId, permissions: body.permissions, exp: Math.floor(Date.now() / 1000) + body.expiresInMinutes * 60 });
