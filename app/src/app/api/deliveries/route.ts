...
const statusValues = ["queued", "processing", "received", "delivered", "failed", "retrying", "cancelled", "dead_letter"] as const;
...
export async function GET(request: Request) {
  try {
    ...
    if (input.status) add((index) => `e.status = $${index}`, input.status);
    ...
    const rows = await sql.query(`
      select e.id, e.endpoint_id, ep.name as endpoint_name, e.application_id, e.message_id, e.direction, e.provider,
        e.provider_event_id, e.event_type, e.status, e.revenue_at_risk, e.revenue_currency, e.received_at,
        e.request_headers, e.request_body, e.retry_count, e.max_retries, e.next_retry_at, e.last_error,
        e.cancelled_at, e.dead_lettered_at, coalesce(a.attempt_count, 0) as attempt_count,
