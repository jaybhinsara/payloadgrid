...
export async function POST(request: Request) {
  try {
    ...
    if (body.inviteToken) {
      ...
      const [user] = await sql`insert into users (name, email, password_hash) values (${body.name}, ${email}, ${passwordHash}) returning id, name, email`;
      ...
      return NextResponse.json({ ok: true, user: { id: user.id, name: user.name, email: user.email }, joined: true }, { status: 201 });
    }
