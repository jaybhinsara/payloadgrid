...
export async function enforceApiRateLimit(apiKeyId: string) {
  ...
  `;
  if (Number(usage.request_count) > BETA_LIMITS.apiRequestsPerMinute) throw new UsageLimitError("API rate limit exceeded. Retry after the current minute.");
}
...
export async function enforceInboundRateLimit(endpointId: string) {
  ...
  const [usage] = await sql`
    ...
    returning request_count
  `;
  if (Number(usage.request_count) > BETA_LIMITS.inboundRequestsPerMinute) throw new UsageLimitError("Inbound endpoint rate limit exceeded. Retry after the current minute.");
}
...
export async function enforceMonthlyMessageLimit(projectId: string) {
  ...
  `;
  if (Number(usage.count) >= BETA_LIMITS.messagesPerMonth) throw new UsageLimitError("Public beta monthly event limit reached");
}