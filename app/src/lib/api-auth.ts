...
export type ApiKeyContext = { keyId: string; projectId: string; organizationId: string };
...
export async function authenticateApiKey(request: Request): Promise<ApiKeyContext | null> {
  ...
  return { keyId: String(key.id), projectId: String(key.project_id), organizationId: String(key.organization_id) };
}
