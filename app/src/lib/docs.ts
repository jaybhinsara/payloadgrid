...
<PayloadGridPortal url={signedEmbedUrl} />` }] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "account-security", title: "Account verification and sessions", paragraphs: ["Password accounts must verify a reachable email address before the first sign-in. Verification links expire after 24 hours and can be reissued from the verification recovery page. Account security lists active devices and lets a user revoke one session, every other session, or all sessions."], bullets: ["Sessions expire after 30 days and after 7 days without activity.", "Password resets expire after one hour and revoke all existing sessions.", "Unverified accounts that remain abandoned are removed after the configured retention period; the default is 7 days.", "Login, verification, password reset, and session revocation activity appears in private account security history."] }
    ]
  },
  {
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "usage", title: "Plan capacity", paragraphs: ["Accepted-event usage combines outbound messages and inbound events across every project in the workspace for the current month. Endpoint limits remain per project and team limits remain per workspace. Internal retry attempts do not count again."], note: { title: "Audit evidence", body: "Security history records organization-scoped mutations with actor, resource, metadata, and time. Use its 25, 50, 75, or 100 row controls for bounded review." } }
    ]
  },
  {
    ...
    sections: [
      ...
      { id: "plans", title: "Plan capacity", table: { headers: ["Plan", "Accepted events / workspace / month", "Endpoints / project", "Members", "Retention"], rows: [["Free", "10,000", "10", "5", "3 days"], ["Starter · $29 USD/month", "100,000", "30", "10", "14 days"], ["Growth · $149 USD/month", "1,000,000", "100", "30", "30 days"], ["Enterprise", "Agreed capacity", "Agreed capacity", "Agreed capacity", "Agreed period"]] } },
      { id: "limits", title: "Request boundaries", table: { headers: ["Resource", "Boundary"], rows: [["API requests", "Plan-specific per API key / minute"], ["Inbound requests", "Plan-specific per endpoint / minute"], ["Payload", "256 KB per event"], ["Batch API", "100 events and 4 MB per request"], ["Retries", "Not counted again as accepted events"], ["Overages", "Rejected; never silently billed"]] } },
      { id: "billing", title: "Paid-plan activation", paragraphs: ["Pricing defaults to USD. When a supported request region is detected, the pricing page and server-created checkout order use the matching fixed regional price. Unknown regions fall back to USD. Starter and Growth capacity activates only after Razorpay Standard Checkout returns a payment that passes server-side signature, order, amount, currency, and captured-status verification. Checkout purchases one monthly period and does not renew automatically."], note: { tone: "warning", title: "Renewal is manual", body: "Purchase the next monthly period before the current one expires. Enterprise billing and recurring arrangements require a written order." } },
