...
- internal_note` }], note: { title: "Keep contracts aligned", body: "Publish the post-transformation payload shape as the event contract consumed by destinations." } },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "storage", title: "Secret handling", paragraphs: ["PayloadGrid displays a key once and stores only its one-way hash and prefix. Put the complete key in Vercel environment variables or your secret manager."], bullets: ["Never commit pg_live_ values.", "Never send a server API key to a browser.", "Create separate keys for services and environments.", "Revoke a key immediately when ownership or exposure changes."] },
...
<PayloadGridPortal url={signedEmbedUrl} />` }] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "overview-metrics", title: "Interpret overview metrics", bullets: ["Delivery rate measures terminal success, not API acceptance.", "Pending queue combines work still waiting or processing.", "Retrying counts scheduled recovery.", "Dead letter counts unresolved terminal failures.", "Average latency uses the latest attempt for project events.", "Revenue at risk includes unresolved non-delivered events with recognized amount and currency metadata."] },
      { id: "usage", title: "Plan capacity", paragraphs: ["Usage tracks accepted outbound messages plus inbound events for the active project and current month. It is separate from QStash message consumption and database storage."], note: { title: "Audit evidence", body: "Security history records organization-scoped mutations with actor, resource, metadata, and time. Use its 25, 50, 75, or 100 row controls for bounded review." } }
    ]
  },
  {
